...
def file_iterate():
    ...
    #Go through files in data folder to fin which ones that have abbreviation lists in their "back_matter"
    
    abbr_list_art = [] # list with abbreviation list articles in data folder
    for file in os.listdir(data_path):
        if file == ".DS_Store":
            continue
        else:
            with open(data_path + file, encoding='utf-8', mode='r') as currentFile:
                f = json.load(currentFile)
                if f["back_matter"] != []:
                    for i in range(len(f["back_matter"])):  # go through all files in that list and put all the files that give a hit on the keyword "abbreviation" in "back_matter"
                        if "dictionary" in f["back_matter"][i]["section"].lower():
                            abbr_list_art.append(str(file)) 
    print("abbr_list files: ", abbr_list_art)
# None of the articles have their abbreviation lists in their back matter
'''
...
'''
def find_cord_uid(url):
    
    #Returns the cord_uid from the metadata_comm_use_subset_100.csv that corresponds to the url input.
...
def abbrv_metadata(abbrv_urls):
    ...
    Returns a csv file with the metadata coulmns "doi", "pmcid" and "pubmed_id" from the original metadata file.
    '''
